...
pf, cov = probability_of_failure(rvs, m, y -> 0.01 .- y, sim)

println("Probability of failure (MonteCarlo): $pf ($cov)")

sim = SobolSampling(10000)
pf, cov = probability_of_failure(rvs, m, y -> 0.01 .- y, sim)

println("Probability of failure (SobolSampling): $pf ($cov)")
